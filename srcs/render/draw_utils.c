...
void calc_wall_distance_height(t_vars *vars) {
    if (vars->ray.side == 0)
        vars->ray.perpWallDist = (vars->ray.mapX - vars->player.posX + (1 - vars->ray.stepX) / 2) / vars->ray.rayDirX;
    else
        vars->ray.perpWallDist = (vars->ray.mapY - vars->player.posY + (1 - vars->ray.stepY) / 2) / vars->ray.rayDirY;
    ...
    vars->ray.wallX = (vars->ray.side == 0) ? vars->player.posY + vars->ray.perpWallDist * vars->ray.rayDirY : vars->player.posX + vars->ray.perpWallDist * vars->ray.rayDirX;
...
void calc_tex_pos(t_vars *vars) {
    ...
    vars->ray.texPos = (vars->ray.drawStart - SCREEN_HEIGHT / 2 + vars->ray.lineHeight / 2) * vars->ray.step;
}
...
void draw_wall(t_vars *vars, int x) {
    ...
    {
        int texY = (int)vars->ray.texPos & (vars->ray.tex_ptr->height - 1);
        vars->ray.texPos += vars->ray.step;
        int color = get_tex_color(vars->ray.tex_ptr, vars->ray.texX, texY);
        if (vars->ray.side == 1)
