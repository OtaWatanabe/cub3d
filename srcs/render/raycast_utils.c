...
void	init_step_size_dist(t_vars *vars)
{
	...
	{
		vars->ray.stepX = -1;
		vars->ray.sideDistX = (vars->player.posX - vars->ray.mapX)
			* vars->ray.deltaDistX;
	}
	else
	{
		vars->ray.stepX = 1;
		vars->ray.sideDistX = (vars->ray.mapX + 1.0 - vars->player.posX)
			* vars->ray.deltaDistX;
	}
	if (vars->ray.rayDirY < 0)
	{
		vars->ray.stepY = -1;
		vars->ray.sideDistY = (vars->player.posY - vars->ray.mapY)
			* vars->ray.deltaDistY;
	}
	else
	{
		vars->ray.stepY = 1;
		vars->ray.sideDistY = (vars->ray.mapY + 1.0 - vars->player.posY)
			* vars->ray.deltaDistY;
	}
}
...
void	init_ray_vars(t_vars *vars, int x)
{
	...
	//レイの方向ベクトル
	vars->ray.rayDirX = vars->player.dirX + vars->player.planeX
		* vars->ray.cameraX;
	vars->ray.rayDirY = vars->player.dirY + vars->player.planeY
		* vars->ray.cameraX;
	//マップ座標
